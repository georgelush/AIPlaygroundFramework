"""
_studio/ui.py - Gradio UI for the Studio addon.
All UI logic is identical to the original studio.py.
Import changed: uses _studio.registry instead of src.registry.
"""
import gradio as gr
import inspect
import uuid
from _studio.registry import AGENTS, TRACES, META, reload_registry
...
def build_and_launch(port: int = 8000) -> None:
    """Build the Gradio app and launch it on the given port."""

    _initial_session_id = _new_session_id()

    with gr.Blocks(title="Agent Studio") as demo:

        session_id_state = gr.State(_initial_session_id)

        # ── Hidden companion components (state + triggers) — off-screen via CSS ──
        with gr.Column(elem_id="hidden-companions"):
            ...
            session_id_input = gr.Textbox(
                value=_initial_session_id, label="", show_label=False,
    ...
    print(f"[Studio] Ready at http://127.0.0.1:{port}")
    demo.launch(server_port=port, inbrowser=True, css=CSS, theme=gr.themes.Base())
