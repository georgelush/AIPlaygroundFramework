...
def node_approve(state: MessagesState, config: RunnableConfig) -> dict:
    ...
    # LangGraph re-runs node_approve from the top on resume — detect which pass we're on.
    # First pass: Redis record does not exist yet → save it and log the pending state.
    # Resume pass: Redis record already exists  → skip pre-interrupt logging (already shown).
    is_resume = bool(_request_store.exists(f"hr:pending:{request_id}"))
